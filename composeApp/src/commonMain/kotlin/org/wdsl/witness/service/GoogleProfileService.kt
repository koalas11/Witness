...
class GoogleProfileServiceImpl : GoogleProfileService {
    ...
    override suspend fun getProfileInfo(googleOAuth: GoogleOAuth): Result<GoogleProfile> {
        return try {
            HttpClient {
                ...
                val response: HttpResponse = client.get("https://www.googleapis.com/oauth2/v1/userinfo") {
                    url {
                ...
                if (response.status.value == 401) {
                    Log.d(TAG, "Google access token is invalid or expired, message: ${response.bodyAsText()}")
                    return Result.Error(OAuthResultError.InvalidToken)
                }
